...
class Player(models.Model):
    player_name = models.CharField(max_length=100, primary_key=True, unique=True)
...
# max_length is set to 45, as that is the longest word in the English language (see https://en.wikipedia.org/wiki/Longest_word_in_English)
class Gamestate(models.Model):
    word_in_play = models.CharField(max_length=45)

    word_to_guess = models.CharField(max_length=45, default=random.choice(open("words").read().splitlines()).lower())
    ...
    def fill_in_guess(self, guess):
        if guess not in self.guessed_letters:
            ...
            if self.check_guess(guess):
                # create a list of indices of word_in_play to fill in by matching the guess against word_to_guess
                entries = [i for i, entry in enumerate(self.word_to_guess) if entry == guess]
                # for each index generated in the list, fill in its location in word_in_play as 'guess'
                for i in entries:
    ...
    def reset(self):
        ...
        self.word_to_guess = random.choice(open("words").read().splitlines()).lower()
        self.word_in_play = ''.join(['#' for i in self.word_to_guess])
